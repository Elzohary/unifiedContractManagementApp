...
import { TaskUpdate, TaskPriority } from '../../../dashboard-management/models/task.model';
...
export class WorkOrderDetailsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  exportToExcel(): void {
    ...
    try {
      ...
      const workOrderData = {
        'Order Number': this.workOrder.orderNumber,
        'Title': this.workOrder.title,
        'Description': this.workOrder.description,
        'Status': this.workOrder.status,
        'Priority': this.workOrder.priority,
        'Created Date': this.formatDate(this.workOrder.createdDate),
        'Start Date': this.formatDate(this.workOrder.startDate),
        'Due Date': this.formatDate(this.workOrder.dueDate),
        'Completion': `${this.workOrder.completionPercentage}%`,
        ...
        'Client': this.workOrder.client?.name || 'N/A',
        'Contact Person': this.workOrder.client?.contactPerson || 'N/A',
        'Contact Email': this.workOrder.client?.contactEmail || 'N/A',
        'Contact Phone': this.workOrder.client?.contactPhone || 'N/A',
        'Location': this.workOrder.location?.address || 'N/A',
      ...
      link.setAttribute('download', `WorkOrder_${this.workOrder.orderNumber}_Export.csv`);
  ...
  private sendNotifications(remarkData: any, isUpdate = false): void {
    if (!remarkData.peopleInvolved || remarkData.peopleInvolved.length === 0) return;
    
    const action = isUpdate ? 'updated' : 'added';
    const notificationData = {
      title: `Work Order Remark ${action}`,
      message: `A ${remarkData.type} remark was ${action} for Work Order ${this.workOrder?.orderNumber}`,
